  ...
  useEffect(() => {
    ...
    getDoctors()
      .then((apiDocs: Doctor[]) => {
        ...
        // Map doctors from admin dashboard API directly
        const mappedList: VerifiedDoctor[] = apiDocs.map((apiDoc) => {
          ...
          const photo =
            apiDoc.photo || (apiDoc as any).image_url || "/hero/hero1.webp";
          ...
          return {
            ...
            patients_count: apiDoc.patients || 0,
            bio_ar: bio,
            image_url: photo,
            badges_ar: [
              "إشراف طبي مباشر",
        ...
        if (isMounted) setDoctors(mappedList);
      })
      .catch(() => {
        // No fallback to mock data — يجب أن تأتي بيانات الأطباء من الأدمن دشبورد فقط
      })
      .finally(() => {
      ...
      <div className="max-w-6xl mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-white/70 text-sm sm:text-base max-w-2xl mx-auto leading-relaxed">
            كل جلسة IV Therapy تخضع لتقييم طبي شخصي مسبق بإشراف أطبائنا
            المتخصصين لضمان أعلى مستويات الأمان والفعالية.
          </p>
        </div>
